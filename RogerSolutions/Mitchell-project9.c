...
void project8_control(), project8_visualize();
...
void updateCurrentPath(Robot* roger){
  ...
  if (!already_used[ybin][xbin]) {
	...
	path_distances[path_size] = sqrt(pow((x-PATH_STEP_SIZE*grad[0]-x), 2)
					 + pow(y-PATH_STEP_SIZE*grad[1]-y, 2));
...
{ 
  ...
  if (cell_distance((int)((x1 - MIN_X)/XDELTA), 
		    (int)((MAX_Y - y1)/YDELTA), goal[0], goal[1]) < 0.05){
