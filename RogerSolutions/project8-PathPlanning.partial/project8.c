...
Robot *roger;
{
  ...
  next_stepsize = velocity_control(DEFAULT_PATH_STEP, roger);
  roger->base_setpoint[X] = x - next_stepsize*grad[X];
  roger->base_setpoint[Y] = y - next_stepsize*grad[Y];
}
...
//void updateControlStep(double new_setpoint){
//  CONTROL_STEP = new_setpoint;
//}
//
...
double time;
{ 
  void generateCurrentStreamline(), maxVelocities(), VelocityReachability();
  ...
  // MITCHELL stuff  
  omega_n = sqrt(Kp_base_rot/I_BASE);
  generateCurrentStreamline(roger);
  // calculatePathDtheta(roger);
  maxVelocities(roger);
  VelocityReachability(roger);

  // computeNewControlStep(roger)
  follow_path(roger);
}
